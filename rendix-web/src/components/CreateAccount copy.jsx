...
export const CreateAccount = ({ onBack }) => {
  ...
  const handleCreate = async () => {
    ...
    setLoading(true);
    // El correo del administrador viaja en los metadatos del registro: el trigger
    // de la base lo copia al perfil. Guardarlo despues del signUp no funciona,
    // porque todavia no hay sesion activa y el RLS lo bloquea.
    const { error: signUpError } = await supabase.auth.signUp({
      ...
      options: {
        data: {
          nombre,
          correo_administrador: correoAdministrador || null,
        },
      },
    });
    setLoading(false);
    ...
    <PhoneFrame>
      <div className="flex-1 flex flex-col px-7 py-6 overflow-y-auto" style={{ backgroundColor: t.bg }}>
        ...
        <p className="text-[11.5px] mb-5" style={{ color: t.gray }}>
          Ahí llegarán los informes que envíes. Puedes agregarlo o cambiarlo más adelante desde tu perfil.
        </p>
